...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // After getting scan result from AI
      const treatmentText =
        typeof result.treatment === "object"
          ? JSON.stringify(result.treatment)
          : result.treatment;

      // Return the result AND the image path to the frontend
      res.status(200).json({
        ...result,
        imagePath: imageUrl,
        scanId: null,
      ...
      res
        .status(500)
        .json({ error: "AI Analysis failed", details: error.message });
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      // Check if the image was uploaded to Cloudinary
      ...
      // Make sure images are saved to diagnosis results and doesn't dissapper
      const savedScan = await Scan.create({
        ...
        treatment:
          typeof treatment === "object" ? JSON.stringify(treatment) : treatment,
      });
