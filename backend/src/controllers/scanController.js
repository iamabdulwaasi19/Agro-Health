...
exports.analyzePlant = async (req, res) => {
  ...
  // Check if user snaps or uploads right file type
  try {
    if (!req.file) {
    ...
    res
      .status(500)
      .json({ error: "Failed to analyze plant", details: error.message });
  }
};
...
exports.getUserHistory = async (req, res) => {
  try {
    const history = await Scan.find({ userId: req.user.id }).sort({
      createdAt: -1,
    });
