...
const DiagnosisDetails = ({ route, navigation }) => {
  ...
  // 2. Fallback if data is missing (for safety)
  if (!diagnosisData) {
    return (
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {/* Use the actual image captured by the user */}
        <Image source={{ uri: imageUri }} style={styles.heroImage} />

        <View style={styles.contentContainer}>
          <View style={styles.diagnosisCard}>
            <View style={styles.titleRow}>
              {/* Dynamic Title from AI */}
              <Text style={styles.mainTitle}>{diagnosisData.diseaseName}</Text>
              <View style={styles.badge}>
                <Text style={styles.badgeText}>{diagnosisData.confidence}% Confident</Text>
              </View>
            </View>
          ...
          {/* Map through the treatment steps provided by AI */}
          {diagnosisData.treatments.map((item, index) => (
            <TreatmentStep 
              key={index}
              icon="leaf-outline" 
              step={`Step ${index + 1}: ${item.title}`}
          ...
          <View style={styles.preventionContainer}>
              <Text style={styles.sectionTitle}>Long-term Prevention</Text>
              {diagnosisData.prevention.map((item, index) => (
                <PreventionItem 
                  key={index} 
                  number={index + 1} 
                  title={item.title} 
