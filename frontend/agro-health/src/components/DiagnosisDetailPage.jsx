...
export function DiagnosisDetailPage() {
  ...
  // 1. Get the diagnosis data passed from SavedResultsPage or Dashboard
  const diagnosis = location.state?.result;

  // 2. Parse the treatment data (since we saved it as a JSON string in MongoDB)
  const treatment = diagnosis?.treatment ? (
    typeof diagnosis.treatment === 'string' ? JSON.parse(diagnosis.treatment) : diagnosis.treatment
  ) : null;
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-6 lg:p-8 max-w-[1440px] mx-auto w-full">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <h1 className="text-[#1C8C36] text-2xl font-bold">Diagnosis Details</h1>
          </div>
          ...
          <Card className="mb-8">
            <CardHeader className="flex flex-row items-start justify-between">
              <div className="space-y-2">
                ...
                <p className="text-[#4B5563] italic">Scientific identification provided by AI</p>
              </div>
              <Badge className="bg-[#1C8C36] text-white px-4 py-1">Saved Result</Badge>
            </CardHeader>
            <CardContent>
              <div className="grid md:grid-cols-3 gap-6 mb-8 border-b pb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-[#4B5563] uppercase tracking-wider font-semibold">Date Scanned</p>
                    <p className="text-[#1C8C36] font-medium">{new Date(diagnosis.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="bg-green-50 p-2 rounded-full">
                  ...
                  <div>
                    <p className="text-xs text-[#4B5563] uppercase tracking-wider font-semibold">Confidence Level</p>
                    {/* <p className="text-[#1C8C36] font-medium">{diagnosis.confidence}%</p> */}
      ...
      {diagnosis.confidence < 1 
        ...
        : diagnosis.confidence}%
                ...
                <div className="flex gap-2 items-center">
                  <Button variant="outline" size="sm" className="border-[#1C8C36] text-[#1C8C36]">
                    <Download className="h-4 w-4 mr-2" />
                    Download
                  </Button>
                  <Button variant="outline" size="sm" className="border-[#1C8C36] text-[#1C8C36]">
              ...
              <div className="space-y-8">
                {treatment?.immediate_actions && (
                  <div>
                    <h3 className="text-[#1C8C36] text-xl font-semibold mb-4">Immediate Treatment Steps</h3>
                    <ul className="space-y-3">
                ...
                {treatment?.prevention_tips && (
                  <div>
                    <h3 className="text-[#1C8C36] text-xl font-semibold mb-4">Prevention Tips</h3>
                    <ul className="space-y-3">
